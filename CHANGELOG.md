...
### Demos as animated SVG

- The two widget demos are animated SVG rather than GIF: a tenth of the size, sharp text at any
  zoom, and a light and a dark file so `<picture>` serves whichever matches the reader's theme,
  which a GIF cannot do because its palette is baked at export.
- Generated by `tools/gen-demo-svg.py` rather than drawn, so a UI change is an edit and a rerun.
- Each crop mode also has a still, in a `<details>` block, so the modes can be compared on click.
  A README cannot offer a click inside an SVG - an `<img>` never receives pointer events and GitHub
  strips inline `<svg>` from markdown - so disclosure plus a still per state is the only genuinely
  clickable form available. The stills are the animated file frozen at each phase, so they cannot
  drift from the animation.
- The Registry Navigator before and after stay as recordings. Their value is being evidence that
  the plugin does this, and an SVG of them would be an illustration a reader could not tell from a
  capture.
